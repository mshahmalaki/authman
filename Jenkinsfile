pipeline {
    ...
    parameters {
        string(name: "IMAGE_NAME", defaultValue: "devopstest2020/authman", description: "Docker image name")
    ...
    stages {
        stage("Build Image") {
            steps {
                script {
                    DOCKERHUB_IMAGE = docker.build(params.IMAGE_NAME)
                }
            }
        }
        stage("Test") {
